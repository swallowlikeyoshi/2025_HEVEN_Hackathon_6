...
# 1차 테스트 결과 (21:00)
# 우선 라인 스무딩은 매우 잘 됨.
# 하지만 다음 미드포인트를 찾는 과정에서 단순 거리로 찾다 보니 내 뒤에 있는 점을 추종하는 현상 발생.
# 내 앞에 있는 점만 따라가는 로직으로 수정 필요.
# 미드포인트들은 내 차를 기준으로 상대 좌표로 표현됨. 격자 하나는 1m.
#
# ==============================================================================
...
# ==========================================================
    ...
    # ... (기존 import 문 아래에 추가하거나 클래스 내 메서드로 추가)
     ...
     def sort_points_greedy(self, points):
        """
        [수정됨] 내 차 '앞쪽'에 있는 점들 중에서만 시작점을 찾아
        등 뒤의 점을 타겟으로 잡는 '역주행 오류'를 원천 차단함
        """
        if not points:
            return []

        x_car, y_car = self.state[0], self.state[1]
        yaw_rad = np.radians(self.state[2])
        car_dir_vec = np.array([np.cos(yaw_rad), np.sin(yaw_rad)])

        # ===============================================================
        # [핵심 수정] 1. "전방"에 있는 점들 중에서 가장 가까운 점 찾기
        # ===============================================================
        best_start_idx = -1
        min_dist = float('inf')
        ...
        for i, p in enumerate(points):
            # 벡터: 차 -> 점
            vec_to_point = np.array([p.x - x_car, p.y - y_car])
            dist = np.linalg.norm(vec_to_point)

            # 내적(Dot Product)이 양수면 '전방', 음수면 '후방'
            if np.dot(vec_to_point, car_dir_vec) > 0: 
                if dist < min_dist:
                    min_dist = dist
                    best_start_idx = i
        
        # 만약 전방에 점이 하나도 없다면? (매우 드문 경우, 예: 코너 끝에서 시야 잃음)
        # 어쩔 수 없이 전체 중에서 가장 가까운 점 선택 (Fallback)
        if best_start_idx == -1:
             dists = [np.hypot(p.x - x_car, p.y - y_car) for p in points]
             best_start_idx = np.argmin(dists)

        # 시작점 추출
        sorted_points = [points.pop(best_start_idx)]
        
        # 2. 나머지는 기존과 동일 (Chain 연결)
        while points:
            last_pt = sorted_points[-1]
            dists = [np.hypot(p.x - last_pt.x, p.y - last_pt.y) for p in points]
            nearest_idx = np.argmin(dists)
            sorted_points.append(points.pop(nearest_idx))
